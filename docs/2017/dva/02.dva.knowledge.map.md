...
然后你要做的全部事情就是在 Less 文件里写 `.button {...}`，并在组件里通过 `styles.button` 来引用他。
...
详见：[React](https://github.com/reactjs/react-router/blob/master/docs/guides/RouteConfiguration.md)
...
更多详见：[React](https://github.com/reactjs/react-router/blob/master/docs/API.md#injected-props)
...
除 `push(location)` 外还有更多方法，详见 [React](https://github.com/reactjs/react-router-redux#pushlocation-replacelocation-gonumber-goback-goforward)
